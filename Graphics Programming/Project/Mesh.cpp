...
Mesh::Mesh(Vertex* ver, int num_ver, UINT* ind, int num_ind, ID3D11Device* device, std::string naam)
{
	//CreateBuffers(ver, num_ver, ind, num_ind, device);
	vertexBuffer = 0;
	indexBuffer = 0;
	num_indices = num_ind;
	vertices = ver;
	Geometry(ver, device, ind, num_ver);
	obb.CreateFromPoints(obb, num_ver, &vertices->Position, sizeof(Vertex));
	name = naam;
}

Mesh::Mesh(ID3D11Device* device, char* objFile, std::string naam)
{
	name = naam;
	...
	obj.close();
	obb.CreateFromPoints(obb, vertCounter, &GetVertices()->Position, sizeof(Vertex));
	...
	num_indices = vertCounter;
	Geometry(&verts[0], device, &indices[0], vertCounter);
	*/
}

void Mesh::Geometry(Vertex* vertices, ID3D11Device* device, UINT* indices, int num_ver)
{
	...
	vbd.ByteWidth = sizeof(Vertex) * num_ver;       
	...
	ibd.ByteWidth = sizeof(int) * num_indices;         
...
{
	if (vertexBuffer) { return vertexBuffer; };
	return nullptr;
}
...
{
	if (num_indices != 0) { return num_indices; };
	return 0;
}
